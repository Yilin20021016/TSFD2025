...
$type._resolveUrl = function WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('https://ic3mt2022.conf.tw/') + 1);

    if (!url || !url.length) {
        return baseUrl;
    }

    if (url.charAt(0) === 'https://ic3mt2022.conf.tw/') {
        var slashslash = baseUrl.indexOf('://');
        if (slashslash === -1) {
            throw Error.argument("baseUrl", Sys.Res.badBaseUrl1);
        }

        var nextSlash = baseUrl.indexOf('https://ic3mt2022.conf.tw/', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('https://ic3mt2022.conf.tw/');
        if (lastSlash === -1) {
